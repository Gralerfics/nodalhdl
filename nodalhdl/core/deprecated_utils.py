...
def use_dict_object(cls):
    """
        [ Deprecated ] 直接加入易导致命名冲突, 将单独实现 DictObject.
...
def use_setmgr_node(cls):
    """
        [ Deprecated ] 不适合以装饰器和全局管理器的方式实现, 将在 diagram 中单独实现.
    ...
    class SetManager:
        ...
        def add_node_into(self, node, set_id): # 将节点加入指定集合
            if node._setmgr_mgr != self:
                raise SetManagerException(f"Nodes of different types should not be added")
        ...
        def separate_node(self, node): # 单点分离成集
            if node._setmgr_mgr != self:
                raise SetManagerException(f"Node type is not correct")
