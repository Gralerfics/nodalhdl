...
from ..core.structure import Diagram, DiagramTypeException, Structure, Node, StructureGenerationException, operator, RuntimeId
...
class Addition(Diagram): # 带参基本算子示例, 整数加法
    ...
    def deduction(s: Structure, runtime_id: RuntimeId):
        """
        ...
        """
        io = s.ports_inside_flipped
        op1_type, op2_type = io.op1.get_type(runtime_id), io.op2.get_type(runtime_id)
        
        if not op1_type.determined or not op2_type.determined:
            io.res.update_type(runtime_id, Auto)
        else:
            if op1_type.belongs(SInt):
                io.res.update_type(runtime_id, SInt[max(op1_type.W, op2_type.W)])
            else:
                io.res.update_type(runtime_id, UInt[max(op1_type.W, op2_type.W)])
    
    def generation(s: Structure, runtime_id: RuntimeId):
        pass # TODO
