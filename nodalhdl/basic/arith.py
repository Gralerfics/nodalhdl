...
from ..core.structure import Structure, StructuralNodes, RuntimeId, StructureGenerationException
...
def Addition(op1_type: SignalType, op2_type: SignalType) -> Structure:
    ...
    def deduction(io_flipped: StructuralNodes, runtime_id: RuntimeId):
        """
        ...
        """
        op1_type, op2_type = io_flipped.op1.get_type(runtime_id), io_flipped.op2.get_type(runtime_id)
        
        if not op1_type.determined or not op2_type.determined:
            io_flipped.res.update_type(runtime_id, Auto)
        else:
            if op1_type.belongs(SInt):
                io_flipped.res.update_type(runtime_id, SInt[max(op1_type.W, op2_type.W)])
            else:
                io_flipped.res.update_type(runtime_id, UInt[max(op1_type.W, op2_type.W)])
    
    def generation(h: HDLFileModel, io_flipped: StructuralNodes, runtime_id: RuntimeId):
        op1_type, op2_type, res_type = io_flipped.op1.get_type(runtime_id), io_flipped.op2.get_type(runtime_id), io_flipped.res.get_type(runtime_id)
