...
def clamp01(x: ComputeElement):
    assert x.type.belong(SFixedPoint)
    _s = x.s
    
    u = _s.add_substructure(f"clamp01", CustomVHDLOperator(
        {"i": x.type},
        {"o": x.type},
        f"o <= (others => '0') when i(i'high) = '1' else\n" +
        f"     (i'high downto {x.type.W_frac} => '0', others => '1') when i(i'high downto {x.type.W_frac}) /= (i'high downto {x.type.W_frac} => '0') else\n" +
        f"     (i'high downto {x.type.W_frac} => '0') & i({x.type.W_frac - 1} downto 0);",
        _unique_name = f"Clamp01_{x.type}"
    ))
    _s.connect(x.node, u.IO.i)
    return ComputeElement(_s, runtime_node = u.IO.o)

def convert_frag_xy(x: ComputeElement):
    assert x.type.belong(UInt[12])
    _s = x.s
    target_t = SFixedPoint[16, 12]
    
    u = _s.add_substructure(f"fragcoord_value_convertor", CustomVHDLOperator(
        {"i": UInt[12]},
        {"o": target_t},
        f"o({target_t.W_frac + 11} downto {target_t.W_frac}) <= i;\n" +
            f"o({target_t.W - 1} downto {target_t.W_frac + 12}) <= (others => '0');\n" +
            f"o({target_t.W_frac - 1} downto 0) <= (others => '0');",
        _unique_name = f"FragCoordValueConvertor_UInt_12_{target_t}"
    ))
    _s.connect(x.node, u.IO.i)
    return ComputeElement(_s, runtime_node = u.IO.o)

def convert_itime(x: ComputeElement):
    assert x.type.belong(UInt[64])
    _s = x.s
    target_t = SFixedPoint[16, 12]
    ...
    u = _s.add_substructure(f"itime_convertor", CustomVHDLOperator(
        {"i": UInt[64]},
        {"o": target_t},
        f"o <= '0' & i({20 + target_t.W_int - 1} downto {20 - target_t.W_frac});",
        _unique_name = f"ITimeConvertor_UInt_64_{target_t}"
    ))
    _s.connect(x.node, u.IO.i)
    return ComputeElement(_s, runtime_node = u.IO.o)


def calc_mux(e: ComputeElement, v: ComputeElement):
    _s = e.s
    
    u = _s.add_substructure("mux_calc", CustomVHDLOperator(
        {"e": e.type, "v": e.type},
        {"o": e.type},
        f"o <= v when e(e'high) = '1' else std_logic_vector(to_unsigned({1 << e.type.W_frac}, {e.type.W}) - unsigned(v));"
    ))
    _s.connect(e.node, u.IO.e)
    _s.connect(v.node, u.IO.v)
    return ComputeElement(_s, runtime_node = u.IO.o)
