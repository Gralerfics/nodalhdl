...
def shader(iTime_us_u64: ComputeElement, fragCoord_u12: vec2) -> ComputeElement:
    ...
    rampFactor = 0.95 * mux(e[e.type.W - 1], t, 1 - t) - e * e
    mixFactor = clamp((rampFactor << 4) + (rampFactor << 2) + 1, 0, 0.999) + s * 0.1
    
    fragColor = vec4(1 - (mixFactor >> 1), 1 - (mixFactor >> 2), 1, 1)
...
# pipelining
levels, Phi_Gr = pipelining(s, rid, 16, model = "simple")
print("Phi_Gr ~", Phi_Gr)
